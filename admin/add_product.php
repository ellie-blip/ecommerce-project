<?php
// --- CHANGED: Added session and admin check to secure the page ---
session_start();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   // --- CHANGED: Updated target directory to go up one level to images/ ---
   $target_dir = "../images/";
   // -----------------------------------------------------------------------
    ...
    // --- CHANGED: Created a variable for the database path ---
  ...
  if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
    ...
    $stmt = $conn->prepare($sql);
        ...
        $stmt->bind_param(
            ...
            $category_id,
            $subcategory_id,
            // --- CHANGED: Replaced $target_file with $db_image_path ---
            $db_image_path,
            // ----------------------------------------------------------
            $description
        );
